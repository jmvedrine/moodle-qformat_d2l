 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qformat_d2l_qti extends qformat_d2l_base {
    /**
    ...
    protected function readquestions($lines) {
        ...
        $questionsdata = $this->getpath($xml,
                array('questestinterop', '#', 'objectbank', 0, '#', 'section', 0, '#', 'item'),
                array(), false);
    ...
    public function create_raw_question($quest) {
        ...
        $meta = $this->getpath($quest,
                array('#', 'itemmetadata', 0, '#', 'qtimetadata', 0, '#', 'qti_metadatafield'),
                false, false);
        $rawquestion->qtype = $this->find_metadata($meta, 'qmd_questiontype');
        $rawquestion->defaultmark = $this->find_metadata($meta, 'qmd_weighting', 1);
            ...
            switch($rawquestion->qtype) {
                ...
                case 'True/False':
                    $rawchoices = $this->getpath($pblock,
                            array('#', 'flow', 0, '#', 'response_lid', 0, '#', 'render_choice',
                                    0, '#', 'flow_label'),
                            array(), false);
                    $choices = array();
        ...
        if ($rawquestion->qtype != 'Long Answer') {
    ...
    /**
     * Extract information for metadata part of question XML.
     * @param array $data XML metadata to parse
     * @param string $label label of information to extract.
     * @param string $default defaut value to return if not fond
     * @return string metadata information
     */
     ...
     */
    protected function process_choices($rawchoices, &$choices) {
        foreach ($rawchoices as $rawchoice) {
            $choice = $this->getpath($rawchoice,
                            array('#', 'response_label', 0),
                            array(), false);
     ...
     * @param array $rawsubqestions XML block to parse
     * @param array $subquestions array of subquestions suitable for a matching rawquestion.
     */
    protected function process_subquestions($rawsubqestions, &$subquestions) {
    ...
    public function process_feedback($feedbackset, &$feedbacks) {
        foreach ($feedbackset as $bbfeedback) {
            ...
            $feedback->ident = $this->getpath($bbfeedback,
                    array('@', 'ident'), '', true);
            $text = new stdClass();
            $this->process_block($bbfeedback, $text);
    ...
    public function process_common($quest) {
        ...
        if ($quest->title) {
            ...
            $question->name = $this->create_default_question_name($text,
                    get_string('defaultname', 'qformat_d2l' , $quest->id));
        }
    ...
    /** Process different feebacks
     * @param object $quest rawquestion
     * @param object $question current Moodle question
     * @param boolean $addcombined whether to add combined feedback or not
     */
    ...
    /**
     * Process a feedback from an answer
     * return 'text', 'format' array in all cases even if feedback don't exists
     * @param object $quest rawquestion
     * @param string $ident identifier of the response for the feedback.
     * @return array cleaned text field.
     */
    ...
    /**
     * Process a multichoice single answer question
    ...
    protected function process_mc($quest, &$questions) {
        ...
        foreach ($responses as $response) {
            ...
            $feedbackids[$response->ident[0]] = $response->feedback;
        }
        ...
        foreach ($quest->choices as $cid => $choice) {
            ...
            $question->feedback[$i] = $this->process_answer_feedback($quest, $feedbackids[$choice->ident]);
            $i++;
        }
    ...
    public function process_matching($quest, &$questions) {
        ...
        if ($subquestioncount < 2 || $subanswercount < 3) {
                $this->error(get_string('notenoughtsubans', 'qformat_d2l', $question->questiontext));
        } else {
            $questions[] = $question;
        }
    }
    ...
    protected function process_tf($quest, &$questions) {
        ...
        foreach ($responses as $response) {
            if ($response->title == 'correct') {
                ...
                $correctfeedbackid = $response->feedback;
                foreach ($quest->choices as $cid => $choice) {
                ...
                $incorrectfeedbackid = $response->feedback;
            }
        }
        ...
        if ($correct) {  // True is correct.
            $question->answer = 1;
            $question->feedbacktrue = $this->process_answer_feedback($quest, $correctfeedbackid);
            $question->feedbackfalse = $this->process_answer_feedback($quest, $incorrectfeedbackid);
        } else {  // False is correct.
            $question->answer = 0;
            $question->feedbacktrue = $this->process_answer_feedback($quest, $incorrectfeedbackid);
            $question->feedbackfalse = $this->process_answer_feedback($quest, $correctfeedbackid);
        }
    ...
    /**
     * Construct the fragment to be included in a cloze question text for a subquestion.
     * @param string $placeholder placeholder for the subquestion in question's text
     * @param array $choices choices for the subquestion
     * @param array $responses responses for the subquestion
     * @param string $subqtype type of subquestion (fib or dropdown)
     * @return string text for the subquestion
     */
    ...
    /**
     * Process multichoice multianswer Questions
     * Parse a multichoice multianswer rawquestion and add the result
     * to the array of questions already parsed.
     * @param object $quest rawquestion
     * @param array $questions array of Moodle questions already done.
     * @param string $questiontype type of question (fib, dropdown) to create
     */
     ...
     * Parse a numerical rawquestion and add the result
     * to the array of questions already parsed.
     * @param object $quest rawquestion
     * @param array $questions array of Moodle questions already done.
     */
     ...
     * Parse a calculated rawquestion and add the result
     * to the array of questions already parsed.
     * @param object $quest rawquestion
     * @param array $questions array of Moodle questions already done.
     */
